...
HAC_LAGS = 5
ROLL = 126
OUT = Path("data/processed/ai_betas.parquet")
...
def main() -> int:
    rets = pd.read_parquet("data/processed/prices.parquet")
    rets.index = pd.to_datetime(rets.index)
    if getattr(rets.index, "tz", None) is not None:
        rets.index = rets.index.tz_localize(None)
    fac = pd.read_parquet("data/processed/ai_factor.parquet")
    ...
    roll.to_parquet("data/processed/ai_betas_rolling.parquet")
    print("wrote data/processed/ai_betas_rolling.parquet")
    return 0
